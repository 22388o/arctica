...
mkdir --parents /mnt/ramdisk/CDROM/shards

#stage transfer CD with shards from current SD and from existing transfer CD ISO if applicable
cp -R /media/$USER/CDROM/* /mnt/ramdisk/CDROM

#create transferCD config
echo "type=transfercd" > /mnt/ramdisk/CDROM/config.txt

#collect shards from sd card for export to transfer CD
cp -r /home/$USER/shards/* /mnt/ramdisk/CDROM/shards

#create iso from transferCD dir
genisoimage -r -J -o /mnt/ramdisk/transferCD.iso /mnt/ramdisk/CDROM
...
wodim dev=$OUTPUT -v -data /mnt/ramdisk/CDROM.iso
